...
const V=(...v)=>new T.Vector3(...v),UP=V(0,1,0),FORWARD=V(0,0,-1);
function gunGeometry(){
  const box=new T.BoxGeometry(1,1,1),tube=new T.CylinderGeometry(1,1,1,16),parts=[];
  const add=(size,at,color,geo=box,rot=[0,0,0])=>parts.push({size,at,color,geo,rot});
  add([.039,.047,.225],[0,.04,-.13],0x7c8b94);add([.046,.027,.2],[0,.008,-.117],0x293943);
  ...
  add([.009,.013,.013],[0,.068,-.23],0xb9d6d7);add([.036,.012,.017],[0,.069,-.045],0x253139);
  add([.007,.008,.008],[0,.075,-.23],0xe1f3b7);
  add([.028,.008,.06],[0,-.044,-.073],0x667780);add([.029,.05,.008],[0,-.017,-.105],0x667780);add([.009,.034,.009],[0,-.013,-.063],0x9eafb5,box,[-.25,0,0]);
  for(const side of [-1,1]){for(let j=0;j<6;j++)add([.0015,.025,.003],[side*.0202,.043,-.054-j*.007],0x344651);for(let j=0;j<5;j++)add([.0015,.004,.026],[side*.018,-.025-j*.012,-.012],0x465963);}
...
export class Weapons {
  constructor(scene,boxes,npcs,onFire=()=>{}){
    ...
    this.ray=new T.Ray();this.hit=V();this.muzzle=V();this.look=new T.Matrix4();this.up=V();this.nextEffect=0;this.items=[];
    ...
    for(let i=0;i<20;i++){
      ...
      const mesh=new T.Mesh(geometry,material);mesh.name='pickup-pistol-'+i;mesh.position.copy(home);mesh.castShadow=mesh.receiveShadow=true;scene.add(mesh);
      ...
      this.items.push({mesh,halo,home,p:home.clone(),v:V(),spin:V(),state:'floating',owner:null,age:0,lastShot:-10,kick:0});
    }
  ...
  input(hand,{position,quaternion,direction,delta,bodyVelocity,grip,trigger,dt,canGrab=true}){
    ...
    this.aim[hand].copy(direction);this.sync(hand,position,quaternion);
  ...
  sync(hand,position,quaternion){
    const item=this.held[hand];if(!item)return;item.p.copy(position);item.mesh.position.copy(position);this.up.set(0,1,0).applyQuaternion(quaternion);this.look.lookAt(position,position.clone().add(this.aim[hand]),this.up);item.mesh.quaternion.setFromRotationMatrix(this.look);item.mesh.translateZ(item.kick*.018);item.mesh.rotateX(item.kick*.08);item.halo.visible=false;
  }
  fire(hand){
  ...
  update(dt,viewer,paused=false){
    ...
    for(const item of this.items){item.age+=dt;item.kick*=Math.exp(-28*dt);
